...
Existing models (Station, StationAlias, CorridorEdge, TrainLocation) are
preserved with identical field names and to_dict() output so the React/
Cytoscape frontend continues to work without any changes.

New operational models (TimetableRun, TimetableEvent, LiveTrainState,
DisruptionEvent, OperationalSnapshot, DelayPrediction, ReschedulingRun,
ReschedulingAction, DetectedConflict, ModelVersion) are additive and are
used only by the new v1 rescheduling API.

UUID primary keys are stored as VARCHAR(36) so they work with SQLite (for
tests) and PostgreSQL equally.  JSONB-like fields use JSON which the
ORM maps to JSONB on PostgreSQL via Alembic migrations.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# EXISTING MODELS — preserved for frontend compatibility
# ══════════════════════════════════════════════════════════════════════════════
...
class Station(db.Model):
    ...
    category    = db.Column(db.String(5),   nullable=True)
...
class CorridorEdge(db.Model):
    """
    Physical track connections.  Extended with operational safety columns
    (base_run_seconds, min_headway_seconds, capacity, direction_group,
    is_enabled) needed by the rescheduling engine.

    The legacy columns (base_time_min, distance_km, is_bidirectional) and
    the dynamic_cost property are fully preserved for the existing frontend.
    """
    ...
    base_time_min       = db.Column(db.Float,      nullable=False, default=60.0)
    distance_km         = db.Column(db.Float,      nullable=True)
    is_bidirectional    = db.Column(db.Boolean,    nullable=False, default=True)

    # Operational safety columns (nullable with defaults; added in Phase 1)
...
class TimetableEvent(db.Model):
    ...
    run_id              = db.Column(db.String(36), db.ForeignKey("timetable_runs.run_id",
                                   ondelete="CASCADE"), nullable=False)
    station_code        = db.Column(db.String(16), db.ForeignKey("stations.id"), nullable=False)
...
class LiveTrainState(db.Model):
    ...
    last_station_code   = db.Column(db.String(16), db.ForeignKey("stations.id"), nullable=True)
    next_station_code   = db.Column(db.String(16), db.ForeignKey("stations.id"), nullable=True)
    current_segment_id  = db.Column(db.Integer,  db.ForeignKey("corridor_edges.edge_id"),
                                    nullable=True)
...
class DisruptionEvent(db.Model):
    ...
    station_code        = db.Column(db.String(16),  db.ForeignKey("stations.id"), nullable=True)
    connection_id       = db.Column(db.Integer,     db.ForeignKey("corridor_edges.edge_id"),
                                    nullable=True)
...
class ReschedulingAction(db.Model):
    ...
    station_code        = db.Column(db.String(16), db.ForeignKey("stations.id"), nullable=True)
    connection_id       = db.Column(db.Integer,   db.ForeignKey("corridor_edges.edge_id"), nullable=True)
